    					...
    					<table width="100%"  border="0" cellspacing="1" cellpadding="1">
    					  ...
    					  <tr>
    					    <td>
								...
								try {  
									$wsCliente = new SoapClient('http://dirpensiones.no-ip.org/nusoap/ws_saldos.php?wsdl');
									//$wsCliente = new nusoap_client('http://dirpensiones.no-ip.org/nusoap/ws_saldos.php?wsdl', true);
									
									
									
									$tmpEntidades =$wsCliente->entidades('','');
									
									//echo '<h2>Debug</h2>';
									//echo '<pre>' . htmlspecialchars($wsCliente->debug_str, ENT_QUOTES) . '</pre>';
									//var_dump($wsCliente);
									if (!empty($tmpEntidades) && count($tmpEntidades > 0)){
										
										//var_dump($tmpEntidades);
										$xmlEntidades =$tmpEntidades;
										//$xmlEntidades = simplexml_load_string($tmpEntidades) or die("No se pudo obtener informacion");
										
									}
								} catch (SoapFault $E) {
									echo "no se imprime nda";
									  $xmlEntidades="";
								}
								
								$xmlEntidades =$tmpEntidades;
								$NumReg=(int) count($xmlEntidades);
								...
								$client = new nusoap_client('http://dirpensiones.no-ip.org/nusoap/ws_saldos.php?wsdl', true);
								...
								$aEntidades = $client->call("entidades",array("cve"=>'0',"clave"=>'0'));*/
								
								#var_dump($aEntidades);
								#echo count($aEntidades);
								?>																					
								  <select name="dep" id="dep">
									<?
									if ( $NumReg > 0) {
										
										//foreach($xmlEntidades as $iCve=>$sDes){
										foreach($xmlEntidades as $Entidad){
											//var_dump($Entidad);
											
											/*if ($xmlEntidades[$iCve]['clave']==1)
												$sSel='selected="selected"';
											else
												$sSel='';*/
											if ((int)$Entidad->clave==1)
												$sSel='selected="selected"';
											else
												$sSel='';
										?>
											<option <?=$sSel?> value="<?=$Entidad->clave;?>"><?=$Entidad->descripcion?></option>
										<?
    					  ...
    					  <tr>
                            ...
                            <p>&nbsp;Si tienes problemas para consultar tu saldo, comunicate al teléfono <strong>31 31698 Ext. 111 y 114</strong> de 08:30hrs a 15:00hrs. de Lunes a Viernes</p>
                            </td>
    					  </tr>
    				  </table>
                    </form>
