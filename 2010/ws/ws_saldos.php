...
//$namespace='http://10.10.20.181:8081/ws/lib';
#$namespace='http://dirpensiones.dyndns.org/nusoap/';
$namespace='http://10.10.5.42/2010/ws/lib';
...
//$server->configureWSDL('Saldos',  "urn:Saldos");
...
#Informacion del emepleado 
$server->register('iEmpleado_dep',                    			// NOmbre del metodo
	array('noDependencia' => 'xsd:int', 'noControl' => 'xsd:int'),  	// Parametros de entrada: fecha, dep(Dependencia)
	array('return' => 'tns:infoEmpleado'),    					// Parametros de salida
	'urn:infoEmpleadowsdl',                         			// namespace
	'urn:infoEmpleadowsdl#iEmpleado_dep',                   		// soapaction
	'rpc',                                    					// style
	'encoded',                               					// use
	'Información general del empleado'        					// documentation
);
...
function iEmpleado_dep($cve_dependencia,$noControl) {		######### Metodo para obtener los datos del empleado
	$iConn = odbc_connect("pensiones","","","");	#Realizo Conexion
...
function usuario($ficha,$passw,$dep) {		#Metodo para realizar la auntentificacion del usuario para el uso de los servicios
	...
	if ($iConn) {
		...
		if ($iNumRegsP > 0 ){			
			...
			elseif (strlen(trim($aDatos->curp))==18 && strlen(trim($passw))==18)
